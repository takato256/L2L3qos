...
def main():
    ...
    parser.add_argument("--vlan", help="VLAN ID", type=int, default=1) 
    ...
    if args.p and args.des and args.m and args.dur:
        ...
        iface = get_if()
        if args.p == 'UDP':
            pkt = Ether(src=get_if_hwaddr(iface), dst="ff:ff:ff:ff:ff:ff") / Dot1Q(vlan=args.vlan) / IP(dst=addr, tos=1) / UDP(dport=4321, sport=1234) / args.m
            pkt.show2()
            try:
